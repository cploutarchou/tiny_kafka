...
impl KafkaProducer {
    ...
    pub async fn send_message(&mut self, topic: &str, partition: i32, message: &str) -> io::Result<()> {
        info!("Sending message to topic: {}, partition: {}", topic, partition);
...
mod tests {
    ...
    async fn test_new_producer_success() {
        ...
        let producer = KafkaProducer::new(addr).await.expect("Failed to create producer");
    ...
    async fn test_send_message() {
        ...
        match result {
            ...
            Err(e) => {
                panic!("Failed to send message: {:?}\nLogs: {}", e, std::panic::Location::caller());
            }
        }
    }
}
